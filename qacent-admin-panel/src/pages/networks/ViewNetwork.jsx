...
export default function ViewNetwork() {
  ...
  const { data: networkData, isLoading, isError } = useGetApiWithIdQuery([
    'networks',
    id,
  ]);
    ...
    <div className="p-6">
      ...
      <div className="mb-8">
        <div className="flex items-center text-sm text-gray-500 mb-2">
          ...
          <span className="mx-2">/</span>
          <span className="text-gray-700">View Network</span>
        </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div className="px-6 py-5 grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8">
          <div>
            <h4 className="text-xs font-medium text-gray-500 uppercase tracking-wider">Network Name</h4>
            <p className="mt-1 text-sm text-gray-900">{network.name}</p>
          </div>

          <div>
            <h4 className="text-xs font-medium text-gray-500 uppercase tracking-wider">Network Code</h4>
            <p className="mt-1 text-sm text-gray-900">{network.code}</p>
          </div>

          <div>
            <h4 className="text-xs font-medium text-gray-500 uppercase tracking-wider">Currency</h4>
            <p className="mt-1 text-sm text-gray-900">
              {network.currency ? `${network.currency.name} (${network.currency.code})` : 'N/A'}
            </p>
          </div>

          <div>
            <h4 className="text-xs font-medium text-gray-500 uppercase tracking-wider">Display Order</h4>
            <p className="mt-1 text-sm text-gray-900">{network.order}</p>
          </div>

          <div>
            <h4 className="text-xs font-medium text-gray-500 uppercase tracking-wider">Extra Field Enabled</h4>
            <p className="mt-1 text-sm text-gray-900">{network.enable_extra_field ? 'Yes' : 'No'}</p>
          </div>

          <div>
            <h4 className="text-xs font-medium text-gray-500 uppercase tracking-wider">Status</h4>
            <p className="mt-1">
              <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusStyle(network.status)}`}>
                {network.status}
              </span>
            </p>
          </div>
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-900">{new Date(network.created_at).toLocaleString()}</p>
          </div>

          <div>
            <h4 className="text-xs font-medium text-gray-500 uppercase tracking-wider">Updated At</h4>
            <p className="mt-1 text-sm text-gray-900">{new Date(network.updated_at).toLocaleString()}</p>
          </div>
        </div>
      </div>
