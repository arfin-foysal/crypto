...
export default function SuspendedUsers() {
  ...
  // Update the query to fetch SUSPENDED users instead of FROZEN
  const { data: users, isLoading, isError } = useGetApiQuery({
    url: `users?page=${page}&search=${search}&role=USER&status=SUSPENDED${minBalance ? `&minBalance=${minBalance}` : ''}${maxBalance ? `&maxBalance=${maxBalance}` : ''}`,
  });
                      ...
                      <div className="flex items-center justify-center space-x-2">
                        ...
                        {user.status === USER_STATUS.SUSPENDED && (
                          <>
                            ...
                            <button
                              onClick={() => handleStatusUpdate(user.id, USER_STATUS.FROZEN)}
